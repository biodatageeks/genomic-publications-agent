...
def analyze_data():
    ...
    plt.tight_layout()
    plt.savefig('comparison_coverage.png', dpi=300)
    ...
    plt.tight_layout()
    plt.savefig('chromosome_distribution.png', dpi=300)
    ...
    # Skrócenie długich nazw chorób
    disease_data.index = [d[:40] + '...' if len(d) > 40 else d for d in disease_data.index]
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('top_diseases.png', dpi=300)
    ...
    plt.axis('equal')
    plt.tight_layout()
    plt.savefig('gene_disease_pairs.png', dpi=300)
    
    print("\nAnalizę zakończono pomyślnie. Wykresy zostały zapisane.")
