...
def clean_rs_id(variant_name: str) -> Optional[str]:
...
def extract_gene_symbol(gene_string: str) -> List[str]:
...
def extract_disease_names(disease_string: str) -> List[str]:
...
def extract_pmids(pmid_string: str) -> List[str]:
...
def transform_ground_truth(confirmed_df: pd.DataFrame, common_pmids: Optional[Set[str]] = None) -> pd.DataFrame:
    """
...
def filter_by_score_threshold(df: pd.DataFrame, threshold: float) -> pd.DataFrame:
    """
    Filtruje DataFrame według progu scoringu.
    
    Args:
        df: DataFrame do filtrowania
        threshold: Minimalny próg scoringu
    ...
    print(f"Filtrowanie relacji z progiem scoringu >= {threshold}...")
    filtered_df = df.copy()
    ...
    has_gene_score = 'gene_score' in filtered_df.columns
    has_disease_score = 'disease_score' in filtered_df.columns
    ...
    for idx, row in filtered_df.iterrows():
        ...
        if has_gene_score and pd.notna(row['genes']) and pd.notna(row['gene_score']):
            ...
            if not any(score >= threshold for score in gene_scores if not pd.isna(score)):
                keep_row = False
        ...
        if has_disease_score and pd.notna(row['diseases']) and pd.notna(row['disease_score']):
            ...
            # Sprawdź, czy którykolwiek ze scorów spełnia warunek
            if not any(score >= threshold for score in disease_scores if not pd.isna(score)):
                keep_row = False
...
def create_coverage_plot(categories: List[str], coverage_values: List[float], output_file: str):
    """Tworzy wykres pokrycia dla różnych kategorii."""
...
def create_top_entities_plot(
    confirmed_data: pd.DataFrame,
    pred_entities: Set[str],
    entity_type: str,
    output_file: str
):
    """Tworzy wykres top 10 encji (genów lub chorób)."""
    plt.figure(figsize=(16, 10))
    
    # Zliczanie występowania encji
    entity_counts = Counter([entity for entities in confirmed_data[f'{entity_type}_list'] for entity in entities])
    top_entities = dict(entity_counts.most_common(10))
    
    # Oznaczenie encji, które są również w przewidzianych relacjach
    entity_keys = list(top_entities.keys())
    entity_values = list(top_entities.values())
    is_in_pred = [entity in pred_entities for entity in entity_keys]
    colors = ['#2ecc71' if in_pred else '#e74c3c' for in_pred in is_in_pred]
    
    plt.bar(entity_keys, entity_values, color=colors)
    plt.title(f'Top 10 najczęściej występujących {entity_type} w potwierdzonych relacjach', fontsize=16)
    plt.xlabel(entity_type.capitalize(), fontsize=14)
...
def create_metrics_plot(
    ...
    """Tworzy wykres metryk (precision, recall, F1)."""
...
def analyze_data(
    ...
    try:
        ...
        # Filtrowanie według progu scoringu
        pred_df = filter_by_score_threshold(pred_df, score_threshold)
        ...
        create_coverage_plot(categories, coverage_values, f"{output_prefix}_comparison_coverage.png")
        
        # 2. Wykres Top 10 genów
        create_top_entities_plot(confirmed_df, pred_genes, 'gene', f"{output_prefix}_top_genes.png")
        
        # 3. Wykres Top 10 chorób
        create_top_entities_plot(confirmed_df, pred_diseases, 'disease', f"{output_prefix}_top_diseases.png")
        ...
        create_metrics_plot(precision_values, recall_values, f1_values, categories, f"{output_prefix}_metrics.png")
